...
class AtomicTweetGenerator(Generator):
    ...
    def generate(self):
        atomic_tweet = {
            'event_id': self.tweet["id_str"],
            'tweet_id': self.tweet["id"],
            'tweet_id_str': self.tweet["id_str"],
            'tweet_text': self.text_generator.generate(),
            'tweet_hashtags': self.entities_attr_extractor.extract("hashtags"),
            'tweet_is_quote': self.tweet["is_quote_status"],
            'tweet_is_retweet': self.retweeted_status in self.tweet,
            'tweet_lang': self.tweet["lang"],
            'tweet_likes': self.retweeted_extractor.extract("favorite_count", self.tweet["favorite_count"]),
            'tweet_mentions': self.entities_attr_extractor.extract("user_mentions"),
            'tweet_retweets': self.tweet["retweet_count"],
            'tweet_source': self.tweet["source"],
            'tweet_streaming': self.streaming,
            'tweet_timestamp': str(self.tweet["created_at"]),
...
class RetweetedTweetGenerator(Generator):
    ...
    def generate(self):
        retweeted_tweet = {
            ...
            'tweet_retweeted_text': self.text_generator.generate(),
...
class QuotedTweetGenerator(Generator):
    ...
    def generate(self):
        quoted_tweet = {
            ...
            'tweet_quoted_text': self.text_generator.generate(),
